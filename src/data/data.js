import gacct from '../images/accountability.jpg';
import edu from '../images/education.jpg';
import health from '../images/health.jpg';
import envt from '../images/environment.jpg';
import sme from '../images/sme.jpg';
import equality from '../images/gender-inclusion.jpg';

export const reviews = [
    {
        name: 'Sahara Reporters',
        comment: `Mauris enim leo, rhoncus sed, vestibulum sit amet, cursus id, 
        turpis. Integer aliquet, massa id lobortis convallis, tortor risus dapibus 
        ugue, vel accumsan tellus nisi eu orci. Mauris lacinia sapien quis libero.`,
        logo: "image.jpeg"

    },
    {
        name: "UNCHR",
        comment: `Nulla ut erat id mauris vulputate elementum. Nullam varius. Nulla
         facilisi.\n\nCras non velit nec nisi vulputate nonummy. Maecenas tincidunt 
         lacus at velit. Vivamus vel nulla eget eros elementum pellentesque.
         \n\nQuisque porta volutpat erat.`,
        logo: "image.jpeg"

    },
    {
        name: "UNICEF",
        comment: `Integer tincidunt ante vel ipsum. Praesent blandit lacinia erat. 
        Vestibulum sed magna at nunc commodo placerat.\n\nPraesent blandit.`,
        logo: "image.jpeg"

    },
    {
        name: "The Guardian",
        comment: `Nulla ut erat id mauris vulputate elementum. Nullam varius. Nulla
         facilisi.\n\nCras non velit nec nisi vulputate nonummy. Maecenas tincidunt 
         lacus at velit.`,
        logo: "image.jpeg"

    },
    {
        name: "The Guardian",
        comment: `Quisque id justo sit amet sapien dignissim vestibulum. Vestibulum
        ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae;
         Nulla dapibus dolor vel est.`,
        logo: "image.jpeg"

    }
]

export const causes = [
    {
        text: "Government \n Accountability",
        img: gacct
    },
    {
        text: "Education",
        img: edu
    },
    {
        text: "Health",
        img: health
    },
    {
        text: "Sustainable \n Environment",
        img: envt
    },
    {
        text: "Small \n Business",
        img: sme
    },
    {
        text: "Gender \n Equality",
        img: equality
    },
]